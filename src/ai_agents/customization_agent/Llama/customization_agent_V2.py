# Reference
# (OpenAI, ChatGPT o1, first prompt, 2025): Guide me in the process to create an AI agent with Llama (run locally with Ollama) that generates a CV customized having the job offer and the actual CV
# (OpenAI, ChatGPT o1, last prompt, 2025): The agent is just making recommendations, it has to generate the new CV rewriting the parts of the CV to have better match with the job post and higlighting the current skills. Without invent any information  
